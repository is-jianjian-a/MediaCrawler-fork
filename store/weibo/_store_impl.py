...
class WeiboDbStoreImplement(AbstractStore):
    ...
    async def get_note_count_by_keyword(self, keyword: str) -> int:
        ...
        async with get_session() as session:
            stmt = select(func.count()).select_from(WeiboNote).where(WeiboNote.source_keyword == keyword)
    ...
    async def get_all_note_ids_by_keyword(self, keyword: str) -> set:
        ...
        async with get_session() as session:
            stmt = select(WeiboNote.note_id).where(WeiboNote.source_keyword == keyword)
            result = await session.execute(stmt)
