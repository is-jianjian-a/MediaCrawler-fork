...
async def update_weibo_note(note_item: Dict):
    ...
    utils.logger.info(f"[store] note_id={note_id} content=\"{save_content_item.get('content', '')[:60]}\" nickname=\"{save_content_item.get('nickname', '')}\" keyword=\"{save_content_item.get('source_keyword', '')}\"")
...
async def update_weibo_note_comment(note_id: str, comment_item: Dict):
    ...
    utils.logger.info(f"[store] comment_id={comment_id} note_id={note_id} nickname=\"{save_comment_item.get('nickname', '')}\" content=\"{save_comment_item.get('content', '')[:40]}\"")
...
async def save_creator(user_id: str, user_info: Dict):
    ...
    utils.logger.info(f"[store] creator user_id={local_db_item.get('user_id')} nickname=\"{local_db_item.get('nickname', '')}\" fans={local_db_item.get('fans', 0)}")
