...
async def update_zhihu_content(content_item: ZhihuContent):
    ...
    utils.logger.info(f"[store] content_id={local_db_item.get('content_id')} title=\"{local_db_item.get('title', '')[:40]}\" content=\"{str(local_db_item.get('content', ''))[:60]}\" type={local_db_item.get('content_type')} keyword=\"{local_db_item.get('source_keyword', '')}\"")
...
async def update_zhihu_content_comment(comment_item: ZhihuComment):
    ...
    local_db_item.update({"last_modify_ts": utils.get_current_timestamp()})
    utils.logger.info(f"[store] comment_id={local_db_item.get('comment_id')} content_id={local_db_item.get('content_id')} content=\"{local_db_item.get('content', '')[:40]}\"")
