...
async def update_douyin_aweme(aweme_item: Dict):
    ...
    utils.logger.info(f"[store] aweme_id={aweme_id} title=\"{save_content_item.get('title', '')[:40]}\" desc=\"{save_content_item.get('desc', '')[:60]}\" keyword=\"{save_content_item.get('source_keyword', '')}\"")
...
async def update_dy_aweme_comment(aweme_id: str, comment_item: Dict):
    ...
    utils.logger.info(f"[store] comment_id={comment_id} aweme_id={aweme_id} nickname=\"{save_comment_item.get('nickname', '')}\" content=\"{save_comment_item.get('content', '')[:40]}\"")
