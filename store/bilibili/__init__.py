...
async def update_bilibili_video(video_item: Dict):
    ...
    utils.logger.info(f"[store] video_id={video_id} title=\"{save_content_item.get('title', '')[:40]}\" desc=\"{save_content_item.get('desc', '')[:60]}\" keyword=\"{save_content_item.get('source_keyword', '')}\"")
...
async def update_up_info(video_item: Dict):
    ...
    utils.logger.info(f"[store] creator user_id={video_item_card.get('mid')} nickname=\"{saver_up_info.get('nickname', '')}\"")
...
async def update_bilibili_video_comment(video_id: str, comment_item: Dict):
    ...
    utils.logger.info(f"[store] comment_id={comment_id} video_id={video_id} nickname=\"{save_comment_item.get('nickname', '')}\" content=\"{save_comment_item.get('content', '')[:40]}\"")
