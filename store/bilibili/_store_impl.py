...
class BiliDbStoreImplement(AbstractStore):
    ...
    async def get_note_count_by_keyword(self, keyword: str) -> int:
        """Get the count of videos with specific source keyword"""
        async with get_session() as session:
            stmt = select(func.count()).select_from(BilibiliVideo).where(BilibiliVideo.source_keyword == keyword)
            result = await session.execute(stmt)
            return result.scalar()

    async def get_all_note_ids_by_keyword(self, keyword: str) -> set:
        """Get all video IDs with specific source keyword"""
        async with get_session() as session:
            stmt = select(BilibiliVideo.video_id).where(BilibiliVideo.source_keyword == keyword)
            result = await session.execute(stmt)
            return {row[0] for row in result.all()}

    async def get_all_existing_ids(self) -> set:
        """Get ALL video IDs in database (for cross-keyword dedup)"""
        async with get_session() as session:
            stmt = select(BilibiliVideo.video_id)
            result = await session.execute(stmt)
            return {row[0] for row in result.all()}
