...
class KuaishouDbStoreImplement(AbstractStore):
    ...
    async def get_note_count_by_keyword(self, keyword: str) -> int:
        ...
        async with get_session() as session:
            stmt = select(func.count()).select_from(KuaishouVideo).where(KuaishouVideo.source_keyword == keyword)
    ...
    async def get_all_note_ids_by_keyword(self, keyword: str) -> set:
        ...
        async with get_session() as session:
            stmt = select(KuaishouVideo.video_id).where(KuaishouVideo.source_keyword == keyword)
            result = await session.execute(stmt)
    ...
    async def get_all_existing_ids(self) -> set:
        ...
        async with get_session() as session:
            stmt = select(KuaishouVideo.video_id)
            result = await session.execute(stmt)
            return {row[0] for row in result.all()}
