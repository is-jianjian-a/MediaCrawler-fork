...
def normalize_image_list(image_list: List[Dict]) -> List[Dict]:
    ...
    The XHS API returns image URLs in both 'url' and 'url_default' fields.
    'url_default' is the preferred high-quality URL, so we use it when available.
    This logic was previously duplicated in core.py and store layer.
    """
    for img in image_list:
        if img.get("url_default") not in ("", None):
...
async def update_xhs_note(note_item: Dict):
    ...
    normalize_image_list(image_list)
    ...
    local_db_item = {
        ...
        "raw_data": json.dumps(note_item, ensure_ascii=False) if not note_item.get("_fallback") else "",  # API raw JSON
    }
    utils.logger.info(f"[store] note_id={note_id} title=\"{local_db_item.get('title', '')[:40]}\" desc=\"{local_db_item.get('desc', '')[:60]}\" type={local_db_item.get('type')} keyword=\"{local_db_item.get('source_keyword', '')}\"")
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    local_db_item = {
        ...
        "raw_data": json.dumps(comment_item, ensure_ascii=False),  # API raw JSON
    }
    utils.logger.info(f"[store] comment_id={comment_id} note_id={note_id} nickname=\"{local_db_item.get('nickname', '')}\" content=\"{local_db_item.get('content', '')[:40]}\"")
...
async def save_creator(user_id: str, creator: Dict):
    ...
    utils.logger.info(f"[store] creator user_id={user_id} nickname=\"{local_db_item.get('nickname', '')}\" fans={local_db_item.get('fans', 0)}")
