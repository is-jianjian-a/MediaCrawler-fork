...
async def update_tieba_note(note_item: TiebaNote):
    ...
    utils.logger.info(f"[store] note_id={save_note_item.get('note_id')} title=\"{save_note_item.get('title', '')[:40]}\" content=\"{str(save_note_item.get('content', ''))[:60]}\" keyword=\"{save_note_item.get('source_keyword', '')}\"")
...
async def update_tieba_note_comment(note_id: str, comment_item: TiebaComment):
    ...
    utils.logger.info(f"[store] comment_id={save_comment_item.get('comment_id')} note_id={note_id} content=\"{save_comment_item.get('content', '')[:40]}\"")
...
async def save_creator(user_info: TiebaCreator):
    ...
    utils.logger.info(f"[store] creator user_id={local_db_item.get('user_id')} nickname=\"{local_db_item.get('nickname', '')}\"")
