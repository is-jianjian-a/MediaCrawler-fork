    ...
    async def search(self):
        ...
        for keyword in config.KEYWORDS.split(","):
            if not keyword.strip():
                utils.logger.info("[KuaishouCrawler.search] Skip empty keyword")
                continue
            ...
            if config.ENABLE_SMART_CRAWLER and is_db_storage(config.SAVE_DATA_OPTION):
                store = kuaishou_store.KuaishouStoreFactory.create_store()
                ...
                if adjusted_max_count <= 0:
                    utils.logger.info(f"[KuaishouCrawler.search] Keyword '{keyword}' already fully crawled, skipping...")
                    continue
            else:
            ...
            while adjusted_max_count > 0 and page <= max_pages:
                ...
                if not videos_res:
                    ...
                    if total_attempted > 0 and failed_count / total_attempted > config.CRAWLER_MAX_FAILURE_RATE:
                        ...
                        utils.logger.error(f"[KuaishouCrawler.search] {stop_reason}, stopping!")
                        break
                    page += 1
                    continue
                ...
                for video_detail in vision_search_photo.get("feeds"):
                    ...
                    if video_id in existing_ids_set:
                        ...
                        continue
                    filtered_feeds.append(video_detail)
                
                if not filtered_feeds:
                    consecutive_empty_pages += 1
                    utils.logger.info(f"[KuaishouCrawler.search] All videos on this page already exist (consecutive: {consecutive_empty_pages}), continuing to next page...")
                    page += 1
                    continue
                ...
                for video_detail in filtered_feeds:
                    video_id = video_detail.get("photo", {}).get("id")
                    video_id_list.append(video_id)
                    total_attempted += 1
                    try:
                        await kuaishou_store.update_kuaishou_video(video_item=video_detail)
                        existing_ids_set.add(video_id)
                        adjusted_max_count -= 1
                        actual_stored_count += 1
                        consecutive_failures = 0
                    except Exception as e:
                        utils.logger.error(f"[KuaishouCrawler.search] Failed to store video {video_id}: {e}")
                        failed_count += 1
                ...
                if adjusted_max_count <= 0:
                    stop_reason = "Reached target count"
                    utils.logger.info(f"[KuaishouCrawler.search] Reached target count: {config.CRAWLER_MAX_NOTES_COUNT}")
                    break
            ...
            utils.logger.info(f"[KuaishouCrawler.search] 📊 FINAL SUMMARY")
            ...
            if actual_new_count < config.CRAWLER_MAX_NOTES_COUNT and stop_reason not in ["Reached target count"]:
                utils.logger.warning("[KuaishouCrawler.search] ⚠️ Did not reach target count!")
    ...
    async def get_video_info_task(
        ...
        async with semaphore:
            try:
                ...
                utils.logger.debug(
                    f"[KuaishouCrawler.get_video_info_task] Get video_id:{video_id} info result"
                )
    ...
    async def get_comments(self, video_id: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            try:
                ...
                await smart_sleep()
                utils.logger.debug(f"Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds before fetching comments for video {video_id}")
