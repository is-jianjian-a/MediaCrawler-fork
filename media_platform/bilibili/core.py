...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def search_by_keywords(self):
        ...
        for keyword in config.KEYWORDS.split(","):
            if not keyword.strip():
                utils.logger.info("[BilibiliCrawler.search_by_keywords] Skip empty keyword")
                continue
            ...
            if config.ENABLE_SMART_CRAWLER and is_db_storage(config.SAVE_DATA_OPTION):
                store = bilibili_store.BiliStoreFactory.create_store()
                ...
                if adjusted_max_count <= 0:
                    utils.logger.info(f"[BilibiliCrawler.search_by_keywords] Keyword '{keyword}' already fully crawled, skipping...")
                    continue
            else:
            ...
            while adjusted_max_count > 0 and page <= max_pages:
                ...
                if not video_list:
                    ...
                    utils.logger.info(f"[BilibiliCrawler.search_by_keywords] No more videos from API for '{keyword}', stopping.")
                    break
                ...
                for video_item in video_list:
                    video_id = video_item.get("aid")
                    if video_id in existing_ids_set:
                        utils.logger.debug(f"Skip existing video: {video_id}")
                        continue
                    filtered_video_list.append(video_item)
                
                if not filtered_video_list:
                    consecutive_empty_pages += 1
                    utils.logger.info(f"[BilibiliCrawler.search_by_keywords] All videos on this page already exist (consecutive: {consecutive_empty_pages}), continuing to next page...")
                    page += 1
                    continue
                ...
                task_list = []
                try:
                    task_list = [self.get_video_info_task(aid=video_item.get("aid"), bvid="", semaphore=semaphore) for video_item in filtered_video_list]
                except Exception as e:
                    failed_count += len(filtered_video_list)
                    total_attempted += len(filtered_video_list)
                    utils.logger.warning(f"[BilibiliCrawler.search_by_keywords] error in the task list. {e}")
                ...
                for video_item in video_items:
                    if video_item:
                        ...
                        try:
                            ...
                            existing_ids_set.add(video_aid)
                            adjusted_max_count -= 1
                            actual_stored_count += 1
                            consecutive_failures = 0
                        except Exception as e:
                            utils.logger.error(f"[BilibiliCrawler.search_by_keywords] Failed to store video {video_aid}: {e}")
                            failed_count += 1
                        ...
                        if consecutive_failures >= config.CRAWLER_MAX_CONSECUTIVE_FAILURES:
                            ...
                            utils.logger.error(f"[BilibiliCrawler.search_by_keywords] {stop_reason}, stopping!")
                            should_stop = True
                            break
                ...
                if adjusted_max_count <= 0:
                    stop_reason = "Reached target count"
                    utils.logger.info(f"[BilibiliCrawler.search_by_keywords] Reached target count: {config.CRAWLER_MAX_NOTES_COUNT}")
                    break
                ...
                api_comment_count += len(video_id_list)
            
            actual_new_count = total_attempted - failed_count
            utils.logger.info("=" * 60)
            utils.logger.info(f"[BilibiliCrawler.search_by_keywords] 📊 FINAL SUMMARY")
            ...
            if actual_new_count < config.CRAWLER_MAX_NOTES_COUNT and stop_reason not in ["Reached target count"]:
                utils.logger.warning("[BilibiliCrawler.search_by_keywords] ⚠️ Did not reach target count!")
    ...
    async def search_by_keywords_in_time_range(self, daily_limit: bool):
        ...
        for keyword in config.KEYWORDS.split(","):
            if not keyword.strip():
                utils.logger.info("[BilibiliCrawler.search_by_keywords_in_time_range] Skip empty keyword")
                continue
    ...
    async def batch_get_video_comments(self, video_id_list: List[str]):
        ...
        if not config.ENABLE_GET_COMMENTS:
            utils.logger.debug(f"[BilibiliCrawler.batch_get_note_comments] Crawling comment mode is not enabled")
            return
    ...
    async def get_comments(self, video_id: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            try:
                utils.logger.info(f"[comments] fetching comments for video_id={video_id}")
                await smart_sleep()
                utils.logger.debug(f"Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after fetching comments for video {video_id}")
    ...
    async def get_creator_videos(self, creator_id: int):
        ...
        while True:
            ...
            if int(result["page"]["count"]) <= pn * ps:
                break
            await smart_sleep()
            utils.logger.debug(f"Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after page {pn}")
            pn += 1
    ...
    async def get_video_info_task(self, aid: int, bvid: str, semaphore: asyncio.Semaphore) -> Optional[Dict]:
        ...
        async with semaphore:
            try:
                ...
                await smart_sleep()
                utils.logger.debug(f"Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after fetching video details {bvid or aid}")
    ...
    async def get_bilibili_video(self, video_item: Dict, semaphore: asyncio.Semaphore):
        ...
        if not config.ENABLE_GET_MEIDAS:
            utils.logger.debug(f"[BilibiliCrawler.get_bilibili_video] Crawling image mode is not enabled")
            return
        ...
        await smart_sleep()
        utils.logger.debug(f"Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after fetching video {aid}")
        if content is None:
            return
