...
class TieBaCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        for keyword in config.KEYWORDS.split(","):
            if not keyword.strip():
                utils.logger.info("[BaiduTieBaCrawler.search] Skip empty keyword")
                continue
            ...
            utils.logger.info(
                f'[search] keyword="{keyword}" | starting search...'
            )
            
            if config.ENABLE_SMART_CRAWLER and is_db_storage(config.SAVE_DATA_OPTION):
                store = tieba_store.TieBaStoreFactory.create_store()
                ...
                if adjusted_max_count <= 0:
                    utils.logger.info(f"[BaiduTieBaCrawler.search] Keyword '{keyword}' already fully crawled, skipping...")
                    continue
            else:
            ...
            while adjusted_max_count > 0 and page <= max_pages:
                ...
                try:
                    ...
                    if not notes_list:
                        ...
                        utils.logger.info(
                            f"[BaiduTieBaCrawler.search] Search note list is empty from API, stopping."
                        )
                        break
                    ...
                    for note_detail in notes_list:
                        ...
                        if note_id in existing_ids_set:
                            ...
                            continue
                        filtered_note_ids.append(note_id)
                    
                    if not filtered_note_ids:
                        consecutive_empty_pages += 1
                        utils.logger.info(f"[BaiduTieBaCrawler.search] All notes on this page already exist (consecutive: {consecutive_empty_pages}), continuing to next page...")
                        page += 1
                        continue
                    
                    utils.logger.info(
                        f"[BaiduTieBaCrawler.search] Note list len: {len(notes_list)}, filtered len: {len(filtered_note_ids)}"
                    )
                    ...
                    note_details = await asyncio.gather(*note_detail_tasks)
                    api_detail_count += len(note_detail_tasks)
                    
                    successful_note_ids = []
                    successful_note_details = []
                    prev_stored = actual_stored_count
                    prev_failed = failed_count
                    for i, note_detail in enumerate(note_details):
                        note_id = filtered_note_ids[i]
                        total_attempted += 1
                        if note_detail is not None:
                            try:
                                ...
                                consecutive_failures = 0
                                successful_note_ids.append(note_id)
                                successful_note_details.append(note_detail)
                            except Exception as e:
                                utils.logger.error(f"[BaiduTieBaCrawler.search] Failed to store note {note_id}: {e}")
                                failed_count += 1
                    ...
                    if consecutive_failures >= config.CRAWLER_MAX_CONSECUTIVE_FAILURES:
                        ...
                        utils.logger.error(f"[BaiduTieBaCrawler.search] {stop_reason}, stopping!")
                        break
                    ...
                    if successful_note_details:
                        await self.batch_get_note_comments(successful_note_details)
                        api_comment_count += len(successful_note_details)
                    
                    page += 1
                    if adjusted_max_count <= 0:
                        stop_reason = "Reached target count"
                        utils.logger.info(f"[BaiduTieBaCrawler.search] Reached target count: {config.CRAWLER_MAX_NOTES_COUNT}")
                        break
            ...
            utils.logger.info(f"[BaiduTieBaCrawler.search] 📊 FINAL SUMMARY")
            ...
            if actual_new_count < config.CRAWLER_MAX_NOTES_COUNT and stop_reason not in ["Reached target count"]:
                utils.logger.warning("[BaiduTieBaCrawler.search] ⚠️ Did not reach target count!")
    ...
    async def get_specified_tieba_notes(self):
        ...
        for tieba_name in config.TIEBA_NAME_LIST:
            ...
            while page_number <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                await smart_sleep()
                utils.logger.debug(f"Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds after processing notes from page {page_number}")
    ...
    async def get_comments_async_task(
        ...
        async with semaphore:
            ...
            await smart_sleep()
            utils.logger.debug(f"Sleeping for {config.CRAWLER_MAX_SLEEP_SEC} seconds before fetching comments for note {note_detail.note_id}")
