...
class TestIpPool(IsolatedAsyncioTestCase):
    ...
    async def test_ip_expiration(self):
        ...
        ip_proxy_info: IpInfoModel = await pool.get_proxy()
        logger.info(f'Obtained proxy: {ip_proxy_info.ip}:{ip_proxy_info.port}')

        # 2. Test non-expired case
        if ip_proxy_info.expired_time_ts:
            logger.info(f'Proxy expiration timestamp: {ip_proxy_info.expired_time_ts}')
            logger.info(f'Current timestamp: {int(time.time())}')
            logger.info(f'Remaining valid time: {ip_proxy_info.expired_time_ts - int(time.time())} seconds')

            is_expired = ip_proxy_info.is_expired(buffer_seconds=30)
            logger.info(f'Is proxy expired (30s buffer): {is_expired}')
            self.assertFalse(is_expired, msg="Newly obtained IP should not be expired")
        else:
            logger.info('Current proxy does not have expiration time set, skipping expiration detection')
        ...
        logger.info(f'Is proxy expired (30s buffer): {is_expired_30s}')
        ...
        logger.info(f'Is proxy expired (30s buffer): {is_expired_critical}')
    ...
    async def test_proxy_pool_auto_refresh(self):
        ...
        logger.info(f'First proxy obtained: {first_proxy.ip}:{first_proxy.port}')

        # Verify current proxy is not expired
        is_expired = pool.is_current_proxy_expired(buffer_seconds=30)
        logger.info(f'Is current proxy expired: {is_expired}')

        if first_proxy.expired_time_ts:
            logger.info(f'Current proxy expiration timestamp: {first_proxy.expired_time_ts}')
            ...
            logger.info(f'Is proxy expired after setting: {is_expired_after}')
    ...
    async def test_ip_expiration_standalone(self):
        ...
        logger.info(f'  Proxy: {ip_no_expire.ip}:{ip_no_expire.port}')
        logger.info(f'  Expiration time: {ip_no_expire.expired_time_ts}')
        ...
        logger.info(f'  Proxy: {ip_valid.ip}:{ip_valid.port}')
        logger.info(f'  Current timestamp: {current_ts}')
        logger.info(f'  Expiration timestamp: {ip_valid.expired_time_ts}')
        logger.info(f'  Remaining time: {ip_valid.expired_time_ts - current_ts} seconds')
        logger.info(f'  Is expired (30s buffer): {is_expired}')
        ...
        logger.info(f'  Proxy: {ip_expired.ip}:{ip_expired.port}')
        logger.info(f'  Current timestamp: {current_ts}')
        logger.info(f'  Expiration timestamp: {ip_expired.expired_time_ts}')
        logger.info(f'  Expired for: {current_ts - ip_expired.expired_time_ts} seconds')
        logger.info(f'  Is expired (30s buffer): {is_expired}')
        self.assertTrue(is_expired, msg="Expired IP should be detected as expired")
        ...
        logger.info(f'  Proxy: {ip_critical.ip}:{ip_critical.port}')
        logger.info(f'  Current timestamp: {current_ts}')
        logger.info(f'  Expiration timestamp: {ip_critical.expired_time_ts}')
        logger.info(f'  Remaining time: {ip_critical.expired_time_ts - current_ts} seconds')
        logger.info(f'  Is expired (30s buffer): {is_expired}')
        ...
        logger.info(f'  Proxy: {ip_just_safe.ip}:{ip_just_safe.port}')
        logger.info(f'  Current timestamp: {current_ts}')
        logger.info(f'  Expiration timestamp: {ip_just_safe.expired_time_ts}')
        logger.info(f'  Remaining time: {ip_just_safe.expired_time_ts - current_ts} seconds')
        logger.info(f'  Is expired (30s buffer): {is_expired}')
        ...
        is_expired = pool.is_current_proxy_expired(buffer_seconds=30)
        logger.info(f'  Is expired when no current proxy: {is_expired}')
        ...
        is_expired = pool.is_current_proxy_expired(buffer_seconds=30)
        logger.info(f'  Is expired after setting valid proxy: {is_expired}')
        ...
        is_expired = pool.is_current_proxy_expired(buffer_seconds=30)
        logger.info(f'  Is expired after setting expired proxy: {is_expired}')
